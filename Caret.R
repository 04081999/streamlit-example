...
install.packages("DALEX",dependencies = T)
...
library(skimr)     # Afficher les Premières statistiques 
library(plotly)    # Visualiser les donnéées( Rendre intéractif les graphes) 
library(lime)      # Expliquer les modeles de machine learning après implémentation
library(DALEX)     # Expliquer les modeles de machine learning après implémentation
...
#Analyse bivariée des variables avec 'gtsummary'-----------------------------------------------------
...
tic()
set.seed(99)
xgbTree = train(churn~. ,data = train_scaled_downSample ,
                method = "xgbTree",
...
tic()
set.seed(99)
svm = train(churn~. ,data = train_scaled_downSample , 
            method = 'svmRadial',
            preProcess = NULL,
            trControl = Control_roc  , 
            tuneLength = 8,
...
set.seed(99)
predicted = predict(objet = xgbTree, test_scaled | select(-churn))

# EVALUATION DES CLASSES PREDITES 
table(predicted)
#"Le model RF a prédit 601 de la classe Non et 65 prédiction de la classe oui"




# Matrice de confusion -------------------------------------------------------------------------
confusion_matrix = confusion_matrix(predicted,
                                    as.factor(test_scaled | pull(churn)),
                                    positive = "Oui", # Classe positive 
                                    mode = "everything") # Afficher toutes les métrics
# Affichage 
confusion_matrix
